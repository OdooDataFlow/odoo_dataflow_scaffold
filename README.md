...
For each model, the files _my_model_csv_fail_ and _my_model_csv_failed.csv_ are created in the folder `data/`. At the end of the load, the files __failed.csv_ contain rejected records that need your attention. If these files are empty, it means all the data was imported.
...
* In `files.py`: the names of the client file and the import file.
    ...
    # Model my.model
    src_my_model = os.path.join(data_src_dir, 'my_model.csv')
    dest_my_model = os.path.join(data_src_dir, 'my_model.csv')
    ```
